...
* ENH: Add xarray-native ``open_datatree`` with ``engine=`` parameter for all 13 backends, enabling ``xd.open_datatree(file, engine="odim")`` and ``xr.open_datatree(file, engine="odim")``. Legacy ``open_*_datatree()`` functions emit ``FutureWarning`` and delegate to the new entry points ({issue}`329`, {pull}`335`) by [@aladinor](https://github.com/aladinor)
